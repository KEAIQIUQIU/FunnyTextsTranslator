...
import subprocess
import requests
from translate_core import trans, COMMON_LANGUAGES

class TranslationApp:
    def __init__(self, root):
        self.root = root
        self.root.title("多轮翻译工具")
        self.root.geometry("900x750")  # 增加高度以容纳新的进度标签

        self.status_var = tk.StringVar(value="就绪")
        self.progress_var = tk.StringVar(value="0/0")
        self.chunk_progress_var = tk.StringVar(value="块: 0/0")  # 新增：块进度变量
    ...
    def create_widgets(self):
        ...
        lang_select_frame = ttk.Frame(input_frame)
        lang_select_frame.pack(fill=tk.X, padx=5, pady=5)
        
        ttk.Label(lang_select_frame, text="源语言:").pack(side=tk.LEFT, padx=5)
        self.source_lang_var = tk.StringVar(value="en")
        self.source_lang_combo = ttk.Combobox(lang_select_frame, textvariable=self.source_lang_var, 
        ...
        ttk.Label(control_frame, text="目标语言:").grid(row=0, column=2, padx=5)
        self.target_lang_var = tk.StringVar(value="zh-Hans")
        self.target_lang_combo = ttk.Combobox(control_frame, textvariable=self.target_lang_var, 
                                             values=COMMON_LANGUAGES, width=15, state="readonly")
        self.target_lang_combo.grid(row=0, column=3, padx=5)
        
        ttk.Label(control_frame, text="分块大小:").grid(row=0, column=4, padx=5)
        self.chunk_size_var = tk.IntVar(value=100)
        ttk.Entry(control_frame, textvariable=self.chunk_size_var, width=5).grid(row=0, column=5, padx=5)

        ttk.Button(control_frame, text="开始翻译", command=self.start_translation).grid(row=0, column=6, padx=10)

        report_frame = ttk.Frame(control_frame)
        report_frame.grid(row=0, column=7, padx=5)
        ...
        # 新增：块进度显示
        ttk.Label(progress_frame, textvariable=self.chunk_progress_var).pack(side=tk.LEFT, padx=10)

        output_frame = ttk.LabelFrame(main_frame, text="实时结果")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if lang_response.status_code == 200:
                    ...
                    # 更新下拉框选项
                    self.source_lang_combo['values'] = supported_languages
    ...
    def start_translation(self):
        ...
        if chunk_size < 10 or chunk_size > 1000:
            messagebox.showwarning("参数错误", "分块大小需在10-1000之间")
            return
        ...
        self.chunk_progress_var.set("块: 0/0")  # 初始化块进度
        self.progress["value"] = 0

        threading.Thread(
            target=self.run_translation,
            args=(input_text, loops, source_lang, target_lang, chunk_size),
            daemon=True
        ).start()

    def run_translation(self, text, loops, source_lang, target_lang, chunk_size):
        try:
            # 新增：块进度回调函数
            def chunk_progress_callback(current, total):
                self.root.after(0, lambda: self.chunk_progress_var.set(f"块: {current}/{total}"))
            ...
            def progress_callback(current, total, result, path):
                ...
                self.root.after(0, lambda: self.status_var.set(f"进度: {current}/{total}轮 当前语言: {'→'.join(path[-3:])}"))

                # 更新输出文本
                self.root.after(0, self.update_output_text, f"第{current}轮: {result}\n\n")

            final_result, all_steps = trans(text, loops, progress_callback, source_lang, target_lang, chunk_size, chunk_progress_callback)
            ...
            self.root.after(0, lambda: self.chunk_progress_var.set("块: 完成"))
            ...
            self.root.after(0, lambda: messagebox.showerror("翻译错误", str(e)))
    ...
    def clear_results(self):
        ...
        self.chunk_progress_var.set("块: 0/0")  # 重置块进度
        self.progress["value"] = 0
